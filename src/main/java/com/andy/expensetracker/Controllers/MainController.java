...
//        showDetailButton.setStyle("-fx-background-color: ##C6D5E7; -fx-text-fill: Black;");
    ...
    private void getAllExpense(){
        ...
        String query="SELECT E.EXPENSE_ID AS ID, format(CREATED_DATE,'yyyy-MM-dd') AS DATE," +
                        "CONCAT(UPPER(SUBSTRING(C.CATEGORY_NAME,1,1)),LOWER(SUBSTRING(C.CATEGORY_NAME,2,len(C.CATEGORY_NAME)))) AS CATEGORY_NAME," +
                        "E.ITEM AS ITEM," +
