...
public class User {
    ...
    public void setUserData(int userId,String username,String passwd){
        this.ID=userId;
    ...
    public void logout(){
        ...
        String updateLoginStatus="UPDATE EX_USER SET ISLOGIN=0 WHERE USER_ID=?";
        try(Connection conn=SQLConn.getConnection();
                PreparedStatement updateStatusStmt=conn.prepareStatement(updateLoginStatus)){

            updateStatusStmt.setInt(1,getUserId());
    ...
    public boolean login(String username,String passwd){
        ...
        query = "select USER_ID,ISLOGIN from EX_USER where username=? and password=?";
...
//            query = "select USER_ID,USERNAME from EX_USER where username=? and password=?";
            ...
            if(result.next()){
                ...
                if(isLogin){
                    ...
                    alert.setContentText("The user has logged in");
                    alert.show();
                    return false;
                }else{
                    String updateLoginStatus="UPDATE EX_USER SET ISLOGIN=1 WHERE USER_ID=?";
                    try(PreparedStatement updateStatusStmt=conn.prepareStatement(updateLoginStatus)){
                        updateStatusStmt.setInt(1,id);
                        updateStatusStmt.executeUpdate();

                        setUserData(id,username,passwd);
                        return true;
                    }
                    catch (SQLException e){
                       e.printStackTrace();
 ...
 public  ArrayList<Category> getCategory(){
        ...
        String query="select category_id, Concat(" +
                ...
                "Lower(substring(category_name,2,len(category_name)))" +
